...
public class ContentPDF {
	...
	public ContentPDF(PictureC picture, String filepath, Integer ctimes)
			throws Exception {
		...
		title1 = new Paragraph("文章分类", fontChinese1);
		...
		System.out.println("cp times:" + ctimes);
	}
	...
	public void writeSimilarity() throws Exception {
		...
		if (filepath != null) {
			...
			if (dir.exists() && dir.isDirectory()) {
				path = new File("").getAbsolutePath() + "\\print\\" + ctimes
						+ "\\" + "文章分类.pdf";
				...
				document.open();
				// 标题
				document.addTitle("文章分类");

				// 作者
				document.addAuthor("肖乐");

				// 主题
				document.addSubject("显示如何对文章进行分类");
				document.addKeywords("文档, 分类");
				document.addCreator("使用iText jar包");
				title2 = new Paragraph("文档相似度", fontChinese2);
				section1 = chapter1.addSection(title2);
				String imagepath = picture.printImage(picture, ctimes);
	...
	public void writeCategory() throws Exception {
		if (filepath != null) {
			...
			if (dir.exists() && dir.isDirectory()) {
				...
				document.open();
				// 标题
				document.addTitle("文档分类");

				// 作者
				document.addAuthor("肖乐");

				// 主题
				document.addSubject("显示如何对文档进行分类");
				...
				nrconn = cc.getContent(filepath, nrconn, ctimes);
				...
				for (int i = 0; i < classes2.length; i++) {
					...
					Set<String> keyset = comparec.extractAKeywords(i,
							parentfile);
